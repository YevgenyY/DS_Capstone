...
The data should be cleaned before modeling. As far as we are going to predict words we should remove numbers, punctuation, URLs, profanity words and stopwords (that come from grammar, i.e. "do", "does", etc.) from texts. As far as we don't want to predict profanity words, we use English profanity words list from this [link]("http://www.bannedwordlist.com/lists/swearWords.csv") and prepare Russian profanity words by hands.
