...
library(RWeka)
bitoken <- function(x) RWeka::NGramTokenizer(x, Weka_control(min=2, max=2))
threetoken <- function(x) NGramTokenizer(x, Weka_control(min=3, max=3))

bigram.en <- TermDocumentMatrix(txt.en,control=list(tokenize = bitoken))
trigram <- TermDocumentMatrix(Jumble, control=list(tokenize = Threesies))
```
